...
// const zch = require('zchaddrjs');
const zch = require('bitcore-lib-zcash')


// // ---------------------------------Create Account----------------------------------------------
router.get("/create_wallet", async function (request, res) {
    let network = zch.Networks.testnet;
    let priv = new zch.PrivateKey(network);
    let add = priv.toAddress().toString(zch.Address.DefaultFormat);


    res.json({
        wallet: {
            privateKey: priv.bn,
            cashAddress: add.replace("zchtest:", "")
        }
    })
});

router.get('/getBalance/:walletAddress', function (req, response) {
    axios.get(`https://explorer.testnet.z.cash/api/addr/${req.params.walletAddress}/?noTxList=1`).then(res => {
...
router.get('/track/:hash', async function (req, response) {
    ...
    axios.get(`https://explorer.testnet.z.cash/api/tx/${req.params.hash}`)
        .then((res) => {
...
router.get('/trackAddress/:walletAddress', function (req, res) {
    axios.get(
        "https://explorer.testnet.z.cash/api/txs/?address=" + req.params.walletAddress
    ).then(transaction =>
        res.status(200).json({
...
router.post('/transfer', function (req, res) {
    ...
    axios.get(`https://explorer.testnet.z.cash/api/addr/${from}/utxo`).then(res => {
        ...
        //   /insight-api/tx/send
        axios.post("https://explorer.testnet.z.cash/api/tx/send", {
            "rawtx": tx.serialize()
        }).then(response => {
            res.json({
