...
router.get("/create_wallet", async function (request, res) {
    let network = litecore.Networks.testnet;
...
router.get('/getBalance/:walletAddress', function (req, response) {
    axios.get(`https://testnet.litecore.io/api/addr/${req.params.walletAddress}/?noTxList=1`).then(res => {
        balance = res.data.balance;
        response.json({
            balance: balance
        })
    }).catch(err => {
        response.json(`Error occured No address found ${err}`)
    })
});
...
router.get('/trackAddress/:walletAddress', function (req, res) {
    axios.get(
        "https://testnet.litecore.io/api/txs/?address=" + req.params.walletAddress
    ).then(transaction =>
        res.status(200).json({
...
router.post('/transfer', function (req, res) {
    ...
    axios.get(`https://testnet.litecore.io/api/addr/${from}/utxo`).then(res => {
        ...
        //   /insight-api/tx/send
        axios.post("https://testnet.litecore.io/api/tx/send", {
            "rawtx": tx.serialize()
        }).then(response => {
            res.json({
