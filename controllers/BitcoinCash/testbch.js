...
router.get('/getBalance/:walletAddress', function (req, response) {
    axios.get(`https://test-bch-insight.bitpay.com/api/addr/${req.params.walletAddress}/?noTxList=1`).then(res => {
...
router.get('/track/:hash', async function (req, response) {
    ...
    axios.get(`https://test-bch-insight.bitpay.com/api/tx/${req.params.hash}`)
        .then((res) => {
...
router.get('/trackAddress/:walletAddress', function (req, res) {
    axios.get(
        "https://test-bch-insight.bitpay.com/api/txs/?address=" + req.params.walletAddress
    ).then(transaction =>
        res.status(200).json({
...
router.post('/transfer', function (req, res) {
    ...
    axios.get(`https://test-bch-insight.bitpay.com/api/addr/${from}/utxo`).then(res => {
